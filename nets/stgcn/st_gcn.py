...
class Model_AE(nn.Module):
    ...
    def __init__(self, in_channels, hidden_channels, hidden_dim, num_class, graph_args,
                 edge_importance_weighting, temporal_kernel_size, **kwargs):
        super().__init__()
    ...
    def forward(self, x):
        ...
        # forward
        # here AA is a fixed graph adj. matrix.
        # A_list = []
        for gcn, importance in zip(self.st_gcn_networks, self.edge_importance):
            x, AA = gcn(x, self.A * importance)
            # print(self.A.shape, AA.shape)
            # A_list.append( AA.detach().data.cpu().numpy() )
...
class Model_AE_wrapper(Model_AE):

    def __init__(self,temporal_kernel_size ):
        graph_args = dict(layout='ntu-rgb+d',strategy='spatial')
        super(Model_AE_wrapper,self).__init__(
            in_channels=3, hidden_channels=3, hidden_dim=3, num_class=60, graph_args=graph_args,
                 edge_importance_weighting=False, temporal_kernel_size = temporal_kernel_size
        )
...
class Model_Reduced_debug(nn.Module):
    r"""Spatial temporal graph convolutional networks."""
...
class Model_thin_returnFeat(nn.Module):
    r"""Spatial temporal graph convolutional networks.
...
class st_gcn(nn.Module):
    ...
    def forward(self, x, A):
        ...
        # print(f"x_in={x0.shape},A={A.shape},x_out={x.shape}")
