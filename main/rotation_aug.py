...
def random_rot_batch(data_batch, theta_val):
    '''
    ...
    for i in range(N):
        theta=np.random.uniform(-theta_val,theta_val,size=3)/180.0*2*np.pi
        rot = get_rot_mat_random(theta)
        rot_list.append(rot)
    rot_list = torch.stack(rot_list, 0)
    # print(rot_list.shape) # (N,3)
    rot_list = rot_list.to(data_batch.device)
    assert rot_list.shape[1]==3 and rot_list.shape[2]==3 and rot_list.shape[0]==N
    res = torch.einsum('nwc,nctvm -> nwtvm', rot_list, data_batch)
    assert res.shape==data_batch.shape 
    return res 



def random_rot_batch_2samples(data_batch, data_batch_2, theta_val):
    '''
    data_batch: (N,C,T,V,M)
    rot_all_list: (N,3,3)
    '''
    ...
    for i in range(N):
        ...
        rot = get_rot_mat_random(theta)
        rot_list.append(rot)
    rot_list = np.stack(rot_list, 0)
    rot_list = torch.FloatTensor(rot_list)
    # print(rot_list.shape) # (N,3)
    rot_list = rot_list.to(data_batch.device)
    assert rot_list.shape[1]==3 and rot_list.shape[2]==3 and rot_list.shape[0]==N
    res = torch.einsum('nwc,nctvm -> nwtvm', rot_list, data_batch)
    res_2 = torch.einsum('nwc,nctvm -> nwtvm', rot_list, data_batch_2)
    assert res.shape==data_batch.shape 
    return res, res_2
